...
class MarsTrendAreaService {
    ...
    def getCategory2MaxmonthScore(String cat1_id) {
        def cat_2 = new ArrayList()
        def cat2_max = sqlClient.client.rows("select avg(price) as price,avg(effect_ln_baseprice) as effect_ln_baseprice,avg(effect_debut) as effect_debut,avg(effect_discount) as effect_discount,avg(effect_coupon) as effect_coupon from  ods.mars_max_month_avg_cat2  where cat1_id = ${cat1_id}")
    ...
    def getProductMaxmonthScore(String cat1_id,String cat2_id) {
        def pro = new ArrayList()
        def pro_max = sqlClient.client.rows("select avg(price) as price,avg(effect_ln_baseprice) as effect_ln_baseprice,avg(effect_debut) as effect_debut,avg(effect_discount) as effect_discount,avg(effect_coupon) as effect_coupon from  ods.mars_max_month_avg_product  where cat1_id = ${cat1_id} and cat2_id= ${cat2_id} ")
